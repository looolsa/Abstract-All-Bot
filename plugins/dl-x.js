...
if (!text) throw `*[ 💠 ] Ingresa un enlace de X/Twitter para proceder con la descarga.*\n\n*⚕️.- Ejemplo: ${usedPrefix + command}* https://x.com/RestrictedVids/status/1810394959100653676?t=6c_mxMFtlgSW-Qtk1sYgdQ&s=19`;
 try {
   const resFG = await twitter(text);
   await m.reply(global.wait);
     const captionFG = '*[ 💠 ] 𝚁𝚎𝚜𝚞𝚕𝚝𝚊𝚍𝚘*';
 ...
 try {
   ...
   for (let x = 0; x < res.media.length; x++) {
     const caption = x === 0 ? res.caption.replace(/https:\/\/t.co\/[a-zA-Z0-9]+/gi, '').trim() : '*[ 💠 ] 𝚁𝚎𝚜𝚞𝚕𝚝𝚊𝚍𝚘*';
     await conn.sendFile(m.chat, res.media[x].url, 'error.mp4', caption, m);
   }
 } catch {
   try {
     const AA = await savefrom(text);
     await conn.sendFile(m.chat, AA.url[0].url, 'error.mp4', '*[ 💠 ] 𝚁𝚎𝚜𝚞𝚕𝚝𝚊𝚍𝚘*', m);
   } catch {
     throw '*[ 🔱 ] Posiblemente el archivo que intentas descargar sea privado, o no esté disponible en este momento. Intenta más tarde.*';
   }
  }
 }
};    
handler.command = /^((twdl|x|twt|twitter)(dl)?)$/i;
