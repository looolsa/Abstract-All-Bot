...
mobile: MethodMobile, 
browser: opcion == '1' ? ['SylphietteBot', 'Safari', '2.0.0'] : methodCodeQR ? ['SylphietteBot', 'Safari', '2.0.0'] : ['Ubuntu', 'Chrome', '20.0.04'],
auth: {
creds: state.creds,
...
global.reloadHandler = async function(restatConn) {
  ...
  conn.bye = '*[ 💠 ] @user, Pendejo se larga.*';
  conn.spromote = '*[ ⚕️ ] @user Ahora forma parte del equipo de administración.*';
  conn.sdemote = '*[ 💠 ] @user Abandona el equipo de administración.*';
  conn.sDesc = '*[ 🔱 ] Se ha modificado la descripción del grupo.*';
  conn.sSubject = '*[ 💠 ] Se ha modificado el nombre del grupo.*\n*Nuevo nombre:* @subject';
  conn.sIcon = '*[ ⚕️ ] Se ha modificado el ícono del grupo.*';
...
setInterval(async () => {
  if (stopped === 'close' || !conn || !conn.user) return;
    const bio = `[ 💠 ] 𝚃𝚒𝚎𝚖𝚙𝚘 𝚊𝚌𝚝𝚒𝚟𝚘: ${uptime} ┃ 𝚆𝚎'𝚛𝚎 𝙰𝚋𝚜𝚝𝚛𝚊𝚌𝚝 - 𝙰𝚕𝚕.`;
...
function clockString(ms) {
  ...
  return [d, ' 𝙳í𝚊(𝚜) ️', h, ' 𝙷𝚘𝚛𝚊(𝚜) ', m, ' 𝙼𝚒𝚗𝚞𝚝𝚘(𝚜) ', s, ' 𝚂𝚎𝚐𝚞𝚗𝚍𝚘(𝚜) '].map((v) => v.toString().padStart(2, 0)).join('');
}
